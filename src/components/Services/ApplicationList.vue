    ...
    async renderApplicationForm(application) {
        ...
        consentFormAlternatives = await Promise.all(
          consentLangBranches.map(async langBranch => {
            return {
              ...
              form: (await renderForm([langBranch.branch.schema_base, ...langBranch.branch.overlays], application.consent_schema.oca_schema_dri)).form
            }
          })
        )
      ...
      try {
        serviceFormAlternatives = await Promise.all(
          serviceLangBranches.map(async langBranch => {
            return {
              language: langBranch.lang,
              form: (await renderForm([langBranch.branch.schema_base, ...langBranch.branch.overlays], application.service_schema.oca_schema_dri)).form
            }
          })
        )
      ...
      return {
        ...
        schema: {
          ...
          answers: application.service_user_data
        },
        usagePolicy
      }
    },
