<template>
  <q-list v-if="consents.length > 0">
    ...
    >
      <q-item-section>
        <q-item-label>{{ consent.label }}</q-item-label>
        <q-item-label v-if='consent.connection' caption>
          to {{ consent.connection.their_label }}
        </q-item-label>
      </q-item-section>
      <q-item-section side>
        <q-btn
  ...
  methods: {
    async renderConsentForm(consent) {
      ...
      try {
        ...
        console.error(e)
        this.$notify.error('Consent form data are corrupted.')
      }
    ...
    async renderServiceForm(consent) {
      ...
      try {
        ...
        console.error(e)
        this.$notify.error('Service form data are corrupted.')
      }
      return {
        form: serviceForm,
